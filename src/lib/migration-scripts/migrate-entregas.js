 ...
 *
 * PARA EXECUTAR:
 * 1. Certifique-se de que as dependências (`firebase-admin`, `dotenv`) estão instaladas.
 * 2. Configure um arquivo .env na raiz do projeto com suas credenciais do Firebase Admin SDK.
...
// Use variáveis de ambiente para não expor as chaves no código
var serviceAccount = {
            ...
            switch (_b.label) {
                case 0:
                    ...
                    return [4 /*yield*/, pipelineResultsRef
                            ...
                            .where('itemCount', '==', null) // Esta é a chave para encontrar os docs antigos
                            .get()];
                case 1:
                ...
                case 2:
                    ...
                    return [4 /*yield*/, pipelineResultsRef.doc(docId).update({
                            itemCount: 0,
                            data: admin.firestore.FieldValue.delete() // Remove o campo antigo
                        })];
                case 3:
                    _b.sent();
                    return [3 /*break*/, 14];
                ...
                case 10:
                    ...
                    _b.label = 11;
                case 11: 
                // 4. Atualiza o documento principal: adiciona itemCount e remove o campo 'data'
                return [4 /*yield*/, pipelineResultsRef.doc(docId).update({
                        itemCount: oldDataArray.length,
                        data: admin.firestore.FieldValue.delete(), // ESSENCIAL: remove o campo antigo
                    })];
                case 12:
                    // 4. Atualiza o documento principal: adiciona itemCount e remove o campo 'data'
                    _b.sent();
